...
async def get_path(player1: int, player2: int, players_to_ignore: str = '', teams_to_ignore: str = '',
    ...
    players_to_ignore = ([int(player) for player in players_to_ignore.split(
        ',')] if players_to_ignore else [])
...
@router.get("/mass_connection_details", response_model=List[MassConnectionSchema])
async def get_mass_connection_details(player_id: int, players: str,
                                      session: Session = Depends(create_session)):
    players = [int(player) for player in players.split(',')]
    return await connectedPlayersService(session).get_mass_connection_details(player_id=player_id, players=players)
